...
const verifyCode = async (req, res) => {
    ...
    if (rows.length == 0) {
        await request(`https://www.fast2sms.com/dev/bulkV2?authorization=n7IPzXKjrQcJfEZhMV4gvkGoqA5R8x3iFsH0W2TumeBpNY1tdbvGx3r2kgaRpXqoV0DnQSYT6P9tzsyd&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
        ...
        let user = rows[0];
        if (user.time_otp - now <= 0) {
            request(`https://www.fast2sms.com/dev/bulkV2?authorization=n7IPzXKjrQcJfEZhMV4gvkGoqA5R8x3iFsH0W2TumeBpNY1tdbvGx3r2kgaRpXqoV0DnQSYT6P9tzsyd&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
                let data = JSON.parse(body);
                if (data.message == 'SMS sent successfully.') {
